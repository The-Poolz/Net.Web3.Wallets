...
namespace Net.Web3.EthereumWallet.Internal
{
    internal class KeccakDigest
    {
        ...
        internal virtual int GetDigestSize()
        {
            return fixedOutputLength >> 3;
        }

        internal virtual void BlockUpdate(byte[] input, int inOff, int len)
        {
            Absorb(input, inOff, len);
        }

        internal virtual int DoFinal(byte[] output, int outOff)
        {
